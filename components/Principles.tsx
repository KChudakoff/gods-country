    ...
    <section className="py-20">
      <div className="max-w-7xl mx-auto px-6">
        {items.map((it, idx) => {
          ...
          return (
            <motion.article key={it.id} className="py-20 border-b border-border flex items-start gap-8" initial="hidden" whileInView="show" viewport={{ once: true }} transition={{ staggerChildren: 0.18 }}>
              <div className="w-1/12">
                <motion.div ref={ref} variants={fadeUp} style={{ y: numY }} className="text-muted text-sm">{it.id}</motion.div>
              </div>

              <div className="w-7/12">
                <motion.h3 variants={fadeUp} style={{ y: titleY }} className="text-5xl font-bold uppercase">{it.title}</motion.h3>
                <motion.p variants={fadeUp} className="mt-6 text-muted max-w-2xl">{it.body}</motion.p>
              </div>

              <div className="w-4/12 text-right">
                <div className="text-sm text-muted">CORE</div>
              </div>
